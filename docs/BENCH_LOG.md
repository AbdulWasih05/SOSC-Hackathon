...
| weather-layer (master) | weather plumbing | 10s | 16 | 2,351,833 | 3072 us | 12288 us | 0 | CONTAMINATED RUN, do not compare: a second palkwatch.exe was running on the same 8-core laptop during this run. NOT a weather regression: `cmd/bench` imports no weather code, so the weather layer is invisible to the benchmark by construction. The weather layer (Open-Meteo sea-state poll) is flag-gated `-weather`, async, and off the per-message hot path; the default build and its hot path are byte-for-byte unchanged. Recorded only to confirm the engine still clears the 50k floor (~47x) under load with the change present. Re-run clean before quoting. |
